...
def single_run_density(Motif, run_id, set_params, density_levels, n_training_epochs,
    ...
    X_, y_ = load_lung_data()
    x_train, x_test, y_train, y_test = train_test_split_normalize(X_, y_, test_size=TEST_SIZE, random_state=run_id)
    ...
    for epsilon in density_levels:
        ...
        set_metrics = set_mlp.fit(x_train, y_train, x_test, y_test, loss=MSE, epochs=n_training_epochs,
...
def lung_train_set_differnt_densities_sequential(Motif=False, runs=10, n_training_epochs=100, set_sparsity_levels=None, use_logical_cores=True):
    # SET model parameters
...
if __name__ == "__main__":
    ...
    if test_density:
        ...
        # [0.065, 0.202, 0.334, 0.468, 0.602, 0.734, 0.863, 0.997, 1.132, 1.265, 1.397, 1.533, 1.669, 1.803, 1.937, 2.066] % density in the first layer
        lung_train_set_differnt_densities_sequential(Motif, runs, n_training_epochs, set_sparsity_levels, use_logical_cores=use_logical_cores)